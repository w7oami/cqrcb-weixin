<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<!DOCTYPE html>
<meta charset="utf-8">
<title>点气球 抽大奖</title>
<meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=0, minimum-scale=1.0, maximum-scale=1.0">
<meta name="format-detection" content="telephone=no">
<meta name="MobileOptimized" content="320">
<link rel="stylesheet" type="text/css" href="${path}/static/css/amazeui.min.css">
<style type="text/css">
    body{font-size:10px;padding:20px 0;}
    p{text-align:justify;}
    .main { max-width: 640px; min-width: 320px; margin: 0 auto;}
    .main div { width: 96%; padding: 0 2%;}
    .main section { font-size: 11px; }
    .main p { font-size: 14px;}
    .main span { font-size: 14px;}
    .main a { font-size: 16px;}
</style>
</head>
<body>
<div class="main">
    <div>
        <section style="text-align: center; font-size: 1em; vertical-align: middle; white-space: nowrap;margin:auto -20px;">
            <section class="wxqq-borderTopColor wxqq-borderBottomColor" style="border-width: 3em; border-style: solid; border-color: rgb(245, 66, 66) transparent; margin: 0px 1em; height: 0px; white-space: nowrap;">
                <br/>
            </section>
            <section style="border-width: 2.6em; border-style: solid; border-color: rgb(255, 255, 255) transparent; margin: -5.6em 1.65em; height: 0px; white-space: nowrap;"></section>
            <section class="wxqq-borderTopColor wxqq-borderBottomColor" style="border-width: 2.2em; border-style: solid; border-color: rgb(245, 66, 66) transparent; margin: 0.9em 2.7em; height: 0px; vertical-align: middle; white-space: nowrap;">
                <section style="padding: 0px 1em; color: white; line-height: 1.5em; overflow: hidden; margin-top: -1.5em; white-space: nowrap; max-height: 4em;background: rgb(245, 66, 66)!important;">
                    重庆农商行成立八周年
                    <br/>参与活动iPhone 6s plus等多重好礼等您来抢！
                </section>
            </section>
        </section>
        <p>
            <br/>
        </p>
        <p class="wxqq-color wxqq-borderLeftColor" style="border-width: 0px 0px 0px 5px; margin: 5px 0px 13px; padding: 0px 10px; color: rgb(245, 66, 66); line-height: 25px; font-family: 微软雅黑; font-size: 16px; border-left-color: rgb(245, 66, 66); border-left-style: solid; white-space: normal; -webkit-font-smoothing: antialiased;">
            活动时间
        </p>
        <p>
            2016年6月29日至7月26日（共四周）。
        </p>
        <p class="wxqq-color wxqq-borderLeftColor" style="border-width: 0px 0px 0px 5px; margin: 5px 0px 13px; padding: 0px 10px; color: rgb(245, 66, 66); line-height: 25px; font-family: 微软雅黑; font-size: 16px; border-left-color: rgb(245, 66, 66); border-left-style: solid; white-space: normal; -webkit-font-smoothing: antialiased;">
            参与对象
        </p>
        <p>
            所有重庆农商行微信公众号用户均可参与。
        </p>
        <p class="wxqq-color wxqq-borderLeftColor" style="border-width: 0px 0px 0px 5px; margin: 5px 0px 13px; padding: 0px 10px; color: rgb(245, 66, 66); line-height: 25px; font-family: 微软雅黑; font-size: 16px; border-left-color: rgb(245, 66, 66); border-left-style: solid; white-space: normal; -webkit-font-smoothing: antialiased;">
            你玩游戏 我送好礼
        </p>
        <p>
            登录“重庆农村商业银行”微信号，即可参加“点气球 抽大奖”在线小游戏。<br>点击“开始游戏”，填写个人姓名和手机号码，即可参加游戏，每人每天限玩3次。每次参与游戏，即可获得抽奖机会哦。
        </p>
        <strong><span class="wxqq-bg" style="padding: 4px 10px; color: rgb(255, 255, 255); font-family: 微软雅黑,Microsoft YaHei; margin-right: 8px; -ms-word-wrap: break-word !important; max-width: 100%; background-color: rgb(245, 66, 66);">活动奖品</span></strong>
        <p style="margin:20px 0px 2px 0px;">
            <strong>一等奖：</strong>Iphone 6s plus（16G）
        </p>
        <p style="text-indent: 4em;margin:2px 0px;">
            每周一台，共抽奖4周
        </p>
        <p style="margin:5px 0px;">
            <strong>二等奖：</strong>价值888元重庆农商行“江渝惠”商城积分
        </p>
        <p style="text-indent: 4em;margin:2px 0px;">
            每周8名，共抽奖4周
        </p>
        <p style="margin:5px 0px;">
            <strong>三等奖：</strong>价值188元重庆农商行“江渝惠”商城积分
        </p>
        <p style="text-indent: 4em;margin:2px 0px 20px 0px;">
            每周88名，共抽奖4周
        </p>
        <strong><span class="wxqq-bg" style="padding: 4px 10px; color: rgb(255, 255, 255); font-family: 微软雅黑,Microsoft YaHei; margin-right: 8px; -ms-word-wrap: break-word !important; max-width: 100%; background-color: rgb(245, 66, 66);">抽奖规则</span></strong>
        <p>
            用户点击“开始游戏”，填写个人姓名和手机号码参加游戏。参与游戏的次数越多，获奖几率就越大哦，每人每天限玩3次游戏。
        </p>
        <p>
            系统将统计一周内所有游戏玩家信息，随机抽取中奖玩家。中奖名单将于每周抽奖后在重庆农商行官方微信和门户网站（www.cqrcb.com）进行公布。每期中奖名单公布后，我行将通过客服热线023-966866与中奖用户进行信息确认。
        </p>
        <strong><span class="wxqq-bg" style="padding: 4px 10px; color: rgb(255, 255, 255); font-family: 微软雅黑,Microsoft YaHei; margin-right: 8px; -ms-word-wrap: break-word !important; max-width: 100%; background-color: rgb(245, 66, 66);">奖品领取</span></strong>
        <p>
            <strong>一等奖领取：</strong>我行客服热线023-966866将与中奖用户确认领奖地址（就近的支行营业部），中奖用户需于2016年8月31日前，携带本人身份证原件前往约定地点领取。
        </p>
        <p>
            <strong>二、三等奖领取：</strong>：每周获奖榜公布后，我行将通过客服热线023-966866与中奖用户进行信息确认。电子商城积分券将在中奖名单公布后7个工作日内，发放到中奖者的电子商城用户ID。如中奖用户尚未注册，我行将自动为您注册商城ID，请您按短信提示登录重庆农商行电子商城，修改用户ID初始密码，即可任意选购，享受积分等额抵价优惠。<span style="color: rgb(255, 0, 0);">1个商城积分=1元现金</span>。
        </p>
        <p>
            活动奖品不折换现金，不单独开具发票，奖品质量及售后服务概由生产商及供应商负责。
        </p>
        <p>
            本活动最终解释权归重庆农村商业银行所有。
        </p>
        <a class="am-btn am-btn-warning am-btn-block am-radius" href="https://open.weixin.qq.com/connect/oauth2/authorize?appid=${appId}&redirect_uri=${basePath}/cqrcb/active&response_type=code&scope=snsapi_base#wechat_redirect">返回活动首页</a>
        <p><br></p>
    </div>

</div>
</body>
</html>


